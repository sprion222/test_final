...
class TextCnnModel(nn.Module):
    def __init__(self):
        ...
        self.num_filter_total = parsers().num_filters * len(parsers().filter_sizes) #TextCnn 的卷积输出*len(TextCnn 的卷积核大小)=2*3
        ...
        ])#1个卷积核, TextCnn的卷积输出维度=2, kernel_size=(2,768)(3,768)(4,768) 因为bert层的输出维度为768
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.filter_list):
            ...
            maxPool = nn.MaxPool2d(
                kernel_size=(parsers().encode_layer - parsers().filter_sizes[i] + 1, 1) #chinese bert 层数-TextCnn 的卷积核大小+1
            )
            # maxPool: [batch_size, channel=2, 1, 1] 最大池化
            pooled = maxPool(out).permute(0, 3, 2, 1)  # 将输出进行换位:[batch_size, h=1, w=1, channel=2]
            pooled_outputs.append(pooled)
        #将结果在第四维上进行拼接
        h_pool = torch.cat(pooled_outputs, len(parsers().filter_sizes))  # len(filter_sizes)=3 [batch_size, h=1, w=1, channel=2 * 3]
...
class BertTextModel_encode_layer(nn.Module):
    def __init__(self):
        ...
        #bert后接一个线形层,bert输出维度为768,转化为14维
    ...
    def forward(self, x):
        ...
        outputs = self.bert(input_ids=input_ids,  #文本的数字化形式, token 在字典中的对应id
                            attention_mask=attention_mask, #实际文本内容的位置赋值为1，而对于填充部分则赋值为0,忽视填充信息
                            token_type_ids=token_type_ids, #token 对应的句子id
                            output_hidden_states=True  # 确保 hidden_states 的输出有值 它的第一个元素是embedding，其余元素是各层的输出，每个元素的形状是[batch_size, sequence_length, hidden]
                            )
        # 取每一层encode出来的向量
        hidden_states = outputs.hidden_states  # 14 * [batch_size, seq_len, hidden] 第一层是 embedding 层不需要
        cls_embeddings = hidden_states[1][:, 0, :].unsqueeze(1)  # [batch_size, 1, hidden] 去掉seq_len

        # 将从第二层开始的每一层的第一个token(cls向量)提取出来，拼在一起当作textCnn的输入
        for i in range(2, 13):
            cls_embeddings = torch.cat((cls_embeddings, hidden_states[i][:, 0, :].unsqueeze(1)), dim=1)
        # cls_embeddings: [batch_size, 12, hidden] 输入textcnn后直接转化为[batch_size, channel=1, 12, hidden]
        pred = self.textCnn(cls_embeddings)
        return pred

#以最后一层的模型输出的隐藏状态作为 TextCNN 模型的输入
class BertTextModel_last_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden_out = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                               output_hidden_states=False) #[batch_size, sequence_length, hidden_size]
        
        #要将bert最后一层的模型输出的隐藏状态输入Textcnn需要修改shape
